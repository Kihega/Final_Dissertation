...
<<<<<<< HEAD
  ...
  const scopeRegions=(dbRegions.length>0?dbRegions:ALL_REGIONS).filter(r=>scope==='national'?true:r.jurisdiction===scope)
  ...
  const availD=dbDistricts.length>0?dbDistricts.map(d=>d.name):getDistricts(region)
  ...
  const availW=dbWards.length>0?dbWards.map(w=>w.name):getVillages(region,district)
